...
import { gravaBloco, jaAnunciado, marcaAnunciado, marcaFalhaDePost, pegaTrava, registraVendas, soltaTrava, ultimoBloco, usandoKv } from './estado.js'
import { contextoDoCiclo, extrasDaVenda } from './mercado.js'
...
async function cicloInterno({ webhook, seco = false } = {}) {
  ...
  /* CONTEXTO DE MERCADO: floor e cotação UMA vez por ciclo — numa varrida de
     40 são 40 anúncios e uma consulta. O floor exclui todos os itens vendidos
     agora (ver `escolheFloor`). Nunca lança: fonte fora vira campo ausente. */
  /* VOLUME DA SEMANA: toda venda com preço em RON entra na soma, anunciada ou
     não (a repetida é o mesmo membro e não conta duas vezes). A hora vem do
     BLOCO, a 2 s cada (medido na Ronin): depois de uma queda o bot anota com
     atraso, mas com a hora certa. ANTES do contexto, pra que o volume do
     rodapé já inclua a venda que está sendo anunciada. Não lança. */
  if (vendas.length && !seco) {
    const agoraS = Math.floor(Date.now() / 1000)
    await registraVendas(
      vendas
        .filter((v) => v.precoWei !== null && v.precoWei !== undefined && v.moeda === 'RON')
        .map((v) => ({ tx: v.tx, id: v.id, precoWei: String(v.precoWei), quando: agoraS - (agora - (v.bloco ?? agora)) * 2 })),
      agoraS,
    )
  }

  const doCiclo = vendas.length && !seco ? await contextoDoCiclo(vendas) : null
  ...
  for (const v of vendas) {
    ...
    // Os extras saem JUNTO com o metadado, não depois: o anúncio espera pelo
    // mais lento dos dois, não pela soma.
    const [meta, extra] = await Promise.all([metadados(v.id), extrasDaVenda(v, doCiclo)])
    if (!primeira) await espera(RESPIRO_MS)
    primeira = false
    const r = await anunciaEmTodos(webhook, v, meta, colecao, extra)
    if (r.ok > 0) {
      anunciadas++
      await marcaAnunciado(v.tx, v.id)
    } else {
      /* NENHUM DESTINO ACEITOU -- E ISSO SUMIA SEM DEIXAR RASTRO.
         ═══════════════════════════════════════════════════════════════════
         O aviso aqui embaixo era `if (r.ok < r.total)`. Quando NÃO HÁ destino
         configurado, `ok` e `total` são os dois 0, e `0 < 0` é falso: não saía
         nem uma linha de log. E o `gravaBloco(agora)` lá embaixo avança o
         ponteiro assim mesmo, então esta venda não volta nunca mais.

         O desfecho era canal mudo, log limpo, ponteiro colado na cabeça e o
         vigia imprimindo "em dia" -- o incidente das 6h23 com outra causa e
         com tudo verde.

         NÃO seguro o ponteiro de propósito: travá-lo num item que o Discord
         recusa em definitivo (embed inválido, webhook revogado) congelaria o
         bot pra sempre, a faixa cresceria ~43.000 blocos por dia, e depois de
         24h o dedup expira e ele republicaria o dia inteiro. O remédio seria
         pior que a doença.
         O que dá pra fazer é não deixar passar calado. A marca abaixo é a
         única memória de que esta venda existiu, e o vigia lê ela. */
      console.warn(`[ciclo] #${v.id}: NENHUM destino aceitou (${r.ok}/${r.total}) -- venda perdida`)
      await marcaFalhaDePost()
    }
    if (r.ok > 0 && r.ok < r.total) console.warn(`[ciclo] #${v.id}: ${r.ok}/${r.total} destinos aceitaram`)
  }
