...
/**
 * ============================================================================
 * LISTINGS — o que já foi anunciado no canal de listings
 * ============================================================================
 *
 * UMA CHAVE POR ORDEM da Ronin Market (`ronkeverse:listing:<id da ordem>`), e
 * não um ponteiro de "maior id visto". As ordens têm id crescente, e o ponteiro
 * seria uma chave só — mas ele pula ordem que fica visível na API fora de
 * ordem, e o caso que produz isso existe: vendedor listando 18 de uma vez
 * (medido em 22/09), com a passada caindo no meio da rajada.
 *
 * A VALIDADE É A DA PRÓPRIA ORDEM, mais um dia. Medido em 22/09: listings de
 * 1, 30, 91 e 181 dias. Um TTL fixo curto faria a chave de uma ordem de 181 dias
 * sumir com ela ainda ativa — e ela seria anunciada de novo na primeira vez que
 * voltasse pra janela (o que acontece sempre que os listings mais novos que ela
 * são vendidos ou cancelados).
 *
 * AQUI NADA FALHA ABERTO, ao contrário do resto do arquivo. Na dedup de vendas
 * falhar aberto custa no máximo UMA repetição, porque o ponteiro de blocos
 * limita a faixa. Aqui não há ponteiro: com o Redis fora, "não sei se anunciei"
 * virando "não anunciei" repostaria a janela INTEIRA a cada minuto. O erro sobe,
 * e `listings.js` aborta a passada sem postar nada.
 *
 * CUSTO: o Redis NÃO é consultado a cada passada — `listings.js` guarda na
 * memória do processo o que já conferiu. Numa passada sem listing novo são zero
 * comandos. O ciclo de vendas no VPS já gasta ~5 por minuto (~220 mil/mês dos
 * 500 mil do plano free); listings a 1 GET por passada somariam mais 43 mil sem
 * necessidade nenhuma.
 */
const PREFIXO_LISTING = 'ronkeverse:listing:'
const CHAVE_INICIO_LISTINGS = 'ronkeverse:listings:inicio'
const UM_DIA_S = 24 * 60 * 60

/** Esta ordem já saiu no canal? LANÇA se o Redis não responder — ver acima. */
export async function listingJaAnunciado(ordem) {
  return (await kv(`get/${encodeURIComponent(PREFIXO_LISTING + ordem)}`)) != null
}

/**
 * Marca a ordem como anunciada até ela expirar (+1 dia de folga).
 *
 * `set/k/1/ex/N` — opções como segmento de caminho. Conferido na REST real em
 * 22/09 antes de escrever esta linha, incluindo o TTL de 180 dias: foi a forma
 * em query string que deixou a dedup de vendas sem existir por semanas.
 */
export async function marcaListing(ordem, expiraEmS, agoraS = Math.floor(Date.now() / 1000)) {
  const ttl = Math.max(UM_DIA_S, Math.floor(expiraEmS - agoraS) + UM_DIA_S)
  await kv(`set/${encodeURIComponent(PREFIXO_LISTING + ordem)}/1/ex/${ttl}`)
}

/**
 * DESDE QUANDO O CANAL CONTA — em ms, ou null se ele nunca rodou.
 *
 * Listing criado antes deste instante nunca é anunciado. A primeira versão
 * "semeava" marcando como visto o que estava na janela, e isso tinha um furo:
 * a janela são os 50 MAIS NOVOS de ~300 ativos. Quando os mais novos são
 * vendidos ou cancelados, listing de semanas atrás sobe pra janela — e, nunca
 * marcado, sairia no canal como novidade. O piso de tempo cobre os 300, os que
 * ainda nem apareceram na janela inclusive, com uma chave só.
 *
 * Valor que não parece timestamp em ms volta como null: um piso no ano de 1970
 * anunciaria a janela inteira, que é exatamente o que a marca existe pra impedir.
 */
export async function inicioDosListings() {
  const v = Number(await kv(`get/${encodeURIComponent(CHAVE_INICIO_LISTINGS)}`))
  return Number.isFinite(v) && v > 1.6e12 ? v : null
}

export async function marcaInicioDosListings(ms) {
  await kv(`set/${encodeURIComponent(CHAVE_INICIO_LISTINGS)}/${ms}`)
}

/*
 * ============================================================================
 * VOLUME DE 7 DIAS — somado das vendas que o próprio bot detecta
 * ============================================================================
 * A Ronke Score API dizia 20.565 RON de volume na semana; as 133 vendas da
 * cadeia no mesmo período somavam 93.399 RON (medido em 24/09/2026). Em vez de
 * trocar por outra fonte de fora, o bot soma o que ELE vê — as mesmas vendas
 * que anuncia, de todas as marketplaces. Cada venda com preço em RON entra num
 * sorted set: pontuação = hora da venda, membro = `tx|id|preço em wei`. Venda
 * vista duas vezes (VPS e Vercel olhando o mesmo bloco) é o mesmo membro, e o
 * ZADD não duplica.
 *
 * COBERTURA: `ronkeverse:volume:desde` diz desde quando o set está completo.
 * Enquanto ele não cobrir os 7 dias inteiros, o volume é null e NÃO SAI — um
 * número parcial com cara de semanal é exatamente o defeito que isto conserta.
 * A primeira semana veio de uma varredura da cadeia (semeadura de 24/09).
 */
const CHAVE_VOLUME = 'ronkeverse:volume:vendas'
const CHAVE_VOLUME_DESDE = 'ronkeverse:volume:desde'
const SEMANA_S = 7 * 24 * 60 * 60
/** Guarda 8 dias: um de folga, pra janela de 7 nunca começar num buraco. */
const GUARDA_S = 8 * 24 * 60 * 60

/** Soma os preços (em wei) dos membros `tx|id|precoWei`. Pura — testada sem Redis. */
export function somaDeVolume(membros) {
  let total = 0n
  for (const m of membros || []) {
    const preco = String(m).split('|')[2]
    if (preco && /^\d+$/.test(preco)) total += BigInt(preco)
  }
  return total
}

/** A cobertura vale se começa antes da janela. Pura — testada sem Redis. */
export function cobreASemana(desdeS, agoraS) {
  return Number(desdeS) > 0 && Number(desdeS) <= agoraS - SEMANA_S
}

/**
 * Anota as vendas do ciclo: `[{ tx, id, precoWei, quando }]`, `quando` em
 * segundos. NÃO LANÇA: o volume é número de rodapé, e a venda tem que sair
 * mesmo com o Redis engasgado.
 */
export async function registraVendas(vendas, agoraS = Math.floor(Date.now() / 1000)) {
  if (!usandoKv || !vendas.length) return
  try {
    const pares = vendas.map((v) => `${Math.floor(v.quando)}/${encodeURIComponent(`${v.tx}|${v.id}|${v.precoWei}`)}`)
    await kv(`zadd/${encodeURIComponent(CHAVE_VOLUME)}/${pares.join('/')}`)
    await kv(`zremrangebyscore/${encodeURIComponent(CHAVE_VOLUME)}/-inf/${agoraS - GUARDA_S}`)
    // A primeira anotação da história marca o começo da cobertura. NX: nunca
    // sobrescreve a data que a semeadura gravou.
    await kv(`set/${encodeURIComponent(CHAVE_VOLUME_DESDE)}/${agoraS}/nx`)
  } catch (e) {
    console.warn('[estado] nao anotei as vendas no volume:', e.message)
  }
}

/** Volume dos últimos 7 dias em wei — ou null se o set ainda não cobre a semana. LANÇA. */
export async function volumeDe7Dias(agoraS = Math.floor(Date.now() / 1000)) {
  if (!usandoKv) return null
  const desde = await kv(`get/${encodeURIComponent(CHAVE_VOLUME_DESDE)}`)
  if (!cobreASemana(desde, agoraS)) return null
  const membros = await kv(`zrangebyscore/${encodeURIComponent(CHAVE_VOLUME)}/${agoraS - SEMANA_S}/${encodeURIComponent('+inf')}`)
  return somaDeVolume(membros)
}

/** Grava a cobertura por cima do que houver. Só a semeadura usa. */
export async function marcaCoberturaDoVolume(desdeS) {
  await kv(`set/${encodeURIComponent(CHAVE_VOLUME_DESDE)}/${Math.floor(desdeS)}`)
}
