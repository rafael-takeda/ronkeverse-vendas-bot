...
const CHAVE_LISTINGS_INICIADOS = 'ronkeverse:listings:iniciados'
...
/**
 * O canal já foi SEMEADO? Na primeira passada da história, os ~300 listings
 * ativos são todos "novos" pro bot; sem esta marca ele despejaria a janela
 * inteira no canal de uma vez. Com ela, a primeira passada só registra o que já
 * existia, e o canal começa a contar dali.
 */
export async function listingsIniciados() {
  return (await kv(`get/${encodeURIComponent(CHAVE_LISTINGS_INICIADOS)}`)) != null
}

export async function marcaListingsIniciados() {
  await kv(`set/${encodeURIComponent(CHAVE_LISTINGS_INICIADOS)}/${Date.now()}`)
}
