...
import { extrasDosListings } from './mercado.js'
import {
  inicioDosListings,
  listingJaAnunciado,
...
export function criaPassadaDeListings({
  ...
  return async function passada({ webhook, seco = false } = {}) {
    ...
    /* FLOOR, DÓLAR, QUANTO O VENDEDOR PAGOU, RONKE SCORE, RARIDADE — só pros
       que vão sair, depois de conferido o Redis. `extrasDosListings` não lança;
       o try é pro dia em que alguém trocar a peça: listing sem extra ainda é
       listing, e ele não pode deixar de sair por causa de enfeite. */
    let extras = []
    try {
      extras = await enriquece(novos)
    } catch (e) {
      console.warn(`[listings] extras falharam, posto sem eles: ${e.message}`)
    }
    const { aceitos, destinos } = await anuncia(webhook, novos, colecao, extras)
