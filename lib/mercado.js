...
export async function contextoDoCiclo(vendas) {
  const [r, colecao] = await Promise.all([
    ouVazio(floorECotacao(vendas.map((v) => String(v.id))), null),
    numerosDaColecao(),
  ])
  // Sem resposta, só vale cotação dos últimos 5 minutos (a memória respeita a
  // validade). Mais velha que isso, o dólar não sai.
  return { ...(r || { floor: null, usd: memCotacao.le('ron') ?? null }), colecao }
}
...
export async function extrasDosListings(listings) {
  ...
  const [ctx, rar, sc, ron, colecao] = await Promise.all([
