 ...
 * por quanto; o resto desta lista mora em três fontes de fora — menos o
 * volume da semana, que o bot soma das próprias vendas (estado.js).
 *
 * ---------------------------------------------------------------------------
...
import { volumeDe7Dias } from './estado.js'
...
/**
 * Holders da coleção, pro rodapé. Foto diária da Ronke Score API. Não lança.
 *
 * CONFERIDO contra fonte independente antes de publicar (24/09): 1.850 na
 * API contra 1.854 donos distintos contando os 6.963 tokens pela GraphQL da
 * Ronin Market — a diferença é a foto diária.
 *
 * O VOLUME DE 7 DIAS DA API fica de fora, e foi medido: ela dizia 20.565 RON
 * (`volume_7d_wron`), e as 133 vendas da cadeia na mesma semana somavam
 * 93.399 RON (Ronin Market 67.906 · RonkeStrategy 17.088 · OpenSea 8.405),
 * todas em RON. Menos de um quarto do real. Chegou a sair no rodapé por
 * algumas horas. O volume que sai agora é o que o bot SOMA das vendas que ele
 * mesmo detecta — ver `volumeDe7Dias` no estado.js e `numerosDoRodape` abaixo.
 */
...
/**
 * Os números do rodapé: holders (Ronke Score API, conferidos contra a Ronin
 * Market) e o volume de 7 dias somado pelo próprio bot — null enquanto a soma
 * ainda não cobre a semana inteira (ver `cobreASemana`). Não lança.
 */
async function numerosDoRodape() {
  const [h, volume] = await Promise.all([numerosDaColecao(), volumeDe7Dias().catch(() => null)])
  const holders = h ? h.holders : null
  if (!holders && volume === null) return null
  return { holders, volume7dWei: volume }
}
