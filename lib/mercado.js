 ...
 *
 * ---------------------------------------------------------------------------
 * AS DUAS FONTES, e o que cada uma garante
 * ---------------------------------------------------------------------------
 * 1. GraphQL da Ronin Market (a mesma de `listings.js`): floor, cotação,
 *    histórico de venda do token e nome de perfil. NÃO É API DOCUMENTADA — ver
 *    o cabeçalho de `listings.js`. Responde sem chave.
 *
 * 2. Ronke Score API (ronke-analytics.vercel.app/api/v1): raridade oficial da
 *    coleção e Ronke Score de carteira. DOCUMENTADA, pública, sem chave, com
 *    versão no caminho (v1 dura 90 dias depois de anunciada a v2). Mas é uma
 *    FOTO DIÁRIA, refeita às 07:00 UTC: o score de quem comprou agora só muda
 *    amanhã, e o `owner` que ela devolve pode estar um dia velho. Por isso
 *    comprador e vendedor vêm SEMPRE da cadeia; daqui só sai o que é estável
 *    num dia — raridade e posição no ranking.
 */
...
/** O nome a mostrar pra um endereço: rótulo conhecido primeiro, perfil depois. */
export function nomeDoEndereco(endereco, perfil) {
  if (!endereco) return null
  return ROTULOS[String(endereco).toLowerCase()] || nomeApresentavel(perfil)
}
...
/** Memória de processo com validade. No VPS o processo vive 24 h; na Vercel, uma invocação. */
...
// Raridade não muda (a API guarda 24 h na borda); score e nome mudam no máximo
// uma vez por dia. A cotação é a única que envelhece rápido.
...
/** Floor (sem os tokens dados) e cotação do RON, numa consulta só. LANÇA. */
async function floorECotacao(excluir) {
  const quantos = Math.min(excluir.length + 1, 50)
  const d = await roninMarket(() =>
    graphql(
      `query($a:String!,$n:Int!){floor:erc721Tokens(tokenAddress:$a,from:0,size:$n,auctionType:Sale,sort:PriceAsc){results{tokenId order{currentPrice paymentToken}}} exchangeRate{ron{usd}}}`,
      { a: COLECAO, n: quantos },
    ),
  )
  const usd = Number(d.exchangeRate && d.exchangeRate.ron && d.exchangeRate.ron.usd)
  if (usd > 0) memCotacao.grava('ron', usd)
  return { floor: escolheFloor(d.floor && d.floor.results, excluir), usd: usd > 0 ? usd : null }
}
...
export async function contextoDoCiclo(vendas) {
  const r = await ouVazio(floorECotacao(vendas.map((v) => String(v.id))), null)
  return r || { floor: null, usd: memCotacao.le('ron') ?? null }
}
...
export async function extrasDaVenda(v, doCiclo = null) {
  ...
  const [hn, rar, sc] = await Promise.all([
  ...
  return {
    ...
    nomeComprador: nomeDoEndereco(v.comprador, hn && hn.perfis.get(baixo(v.comprador))),
    nomeVendedor: vendedor ? nomeDoEndereco(vendedor, hn && hn.perfis.get(baixo(vendedor))) : null,
...
export async function extrasDosListings(listings) {
  ...
  const [ctx, rar, sc] = await Promise.all([
    ouVazio(floorECotacao(ids), null),
    ouVazio(raridades(ids), null),
    ouVazio(scores(vendedores), null),
  ])
  return listings.map((l) => {
    const quem = String(l.vendedor).toLowerCase()
    const perfil = perfis.get(quem)
    return {
      usd: ctx ? ctx.usd : memCotacao.le('ron') ?? null,
      floor: ctx ? ctx.floor : null,
      pagou: historicos.has(String(l.id)) ? compraDoVendedor(historicos.get(String(l.id)), l.vendedor) : null,
      nomeVendedor: nomeDoEndereco(l.vendedor, perfil),
