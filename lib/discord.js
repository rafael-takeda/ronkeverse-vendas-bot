...
function linhaDolar(precoWei, moeda, extra) {
  ...
  if (!(valor > 0)) return ''
  // Com o RON a US$ 0,058 (24/09), quase toda venda fica abaixo de US$ 100 —
  // aí vale o centavo, e com as duas casas: "$29.00", não "$29".
  const casas = valor >= 100 ? { maximumFractionDigits: 0 } : { minimumFractionDigits: 2, maximumFractionDigits: 2 }
  return `\n≈ $${valor.toLocaleString('en-US', casas)}`
}
...
/**
 * A raridade oficial (Ronke Score API), curta: "Rank: #3021". 1/1 fica fora
 * da escada 1..N de propósito — a API não dá rank a eles —, então sai o tier.
 */
function textoDoRank(r) {
  if (!r) return null
  if (r.rank !== null) return `Rank: #${r.rank}`
  ...
  const tipo = { official_1of1: ' official', community_1of1: ' community' }[r.tier] || ''
  return `Rank: 1/1${tipo}`
}

/**
 * O RODAPÉ — floor e rank no lugar onde ficava o horário.
 * ---------------------------------------------------------------------------
 * Até 24/09 floor e raridade eram CAMPOS no corpo ("563 RON (+15%)", "Rank
 * #3,021 / Eyebrow Raised Ronke · 0.89%"), e o listing levava no rodapé a
 * hora em que nasceu. O dono pediu mais limpo: o Discord já mostra a hora da
 * mensagem, e o atraso do bot é de segundos — minutos, no pior caso. Floor e
 * rank viraram duas etiquetas curtas, lado a lado, onde ficava a hora:
 *
 *   Ronin · Ronin Market · 1,850 holders · 93,399 RON 7d volume · FP: 563 RON · Rank: #3021
 *
 * FP é o floor SEM o próprio item (ver `escolheFloor` no mercado.js): num
 * listing mais barato que todos, o leitor vê o preço abaixo do FP — é o
 * "novo floor" dito sem campo.
 */
function rodape(base, extra) {
  const partes = [base]
  // Holders da Ronke Score API (conferidos); volume de 7 dias SOMADO PELO BOT
  // das vendas que ele mesmo detecta (`volume7dWei`, BigInt). O `volume7d` que
  // a API devolve é ignorado de propósito — ver `numerosDaColecao`.
  const c = extra.colecao
  if (c && c.holders) partes.push(`${c.holders.toLocaleString('en-US')} holders`)
  if (c && typeof c.volume7dWei === 'bigint' && c.volume7dWei > 0n) {
    const ron = Math.round(Number(c.volume7dWei / 10n ** 15n) / 1000)
    partes.push(`${ron.toLocaleString('en-US')} RON 7d volume`)
  }
  if (extra.floor) partes.push(`FP: ${precoTexto(extra.floor.precoWei)} RON`)
  const rank = textoDoRank(extra.raridade)
  if (rank) partes.push(rank)
  return partes.join(' · ')
}
...
export function montaEmbed(venda, meta, colecao = 'NFT', extra = {}) {
  ...
  const campos = [
    ...
    // Floor e rank moram no rodapé (ver `rodape`); no corpo só o que pede linha.
    ...[campoPagou(venda.precoWei, venda.moeda, x)].filter(Boolean),
  ]
...
export function montaEmbedDeListing(l, colecao = 'NFT', extra = {}) {
  ...
  return {
    ...
    url: l.link,
    color: COR_LISTING,
    // Primeira fileira igual à de sempre (Price · Seller · Expires); o "Seller
    // paid" desce pra segunda, como na venda. Floor e rank ficam no rodapé.
    fields: [
    ...
    // SEM `timestamp`, por decisão do dono em 24/09. Ele mostrava a hora em que
    // o listing nasceu (útil só se o bot anunciasse com atraso grande depois de
    // uma queda); o Discord já mostra a hora da mensagem, e o lugar no rodapé
    // ficou com o floor e o rank.
  }
}
